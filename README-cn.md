...
我们正式发布 **MiniCPM5** 系列首个模型 **MiniCPM5-1B**。这是面向端侧、本地部署和资源受限场景打造的 1B 稠密 Transformer，在公开评测中达到同尺寸开源模型 SOTA 水平；模型采用标准 `LlamaForCausalLM` 架构，支持原生 128K 上下文，并提供 BF16、GGUF、MLX、AWQ、GPTQ 等多种版本。
...
其余后端（llama.cpp / Ollama / LM Studio / MLX）以及 AWQ / GPTQ 量化权重的 vLLM 推理路线见下方 cookbook 表格。
...
| 顶层 Skill | 作用 | 路由到 |
| --- | --- | --- |
| **[`minicpm5-deploy`](./skills/minicpm5-deploy/SKILL.md)** | 推理路由 | `transformers` · `vllm`（含 AWQ / GPTQ 量化路线）· `sglang` · `llama-cpp` · `ollama` · `lmstudio` · `mlx` |
...
| [vLLM](https://github.com/vllm-project/vllm) | BF16 / FP16 OpenAI server；支持 AWQ / GPTQ-Marlin Int4 量化权重 | [`vllm.md`](./docs/deployment/vllm.md)；量化：[`awq.md`](./docs/deployment/awq.md) / [`gptq.md`](./docs/deployment/gptq.md) | [`minicpm5-deploy-vllm`](./skills/minicpm5-deploy-vllm/SKILL.md)；量化：[`awq`](./skills/minicpm5-deploy-awq/SKILL.md) / [`gptq`](./skills/minicpm5-deploy-gptq/SKILL.md) |

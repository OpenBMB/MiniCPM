...
## 更新日志🔥
- 📌 [2026.05.19] **[MiniCPM5-1B](https://huggingface.co/openbmb/MiniCPM5-1B)** 发布！这是一款面向端侧与资源受限场景的 1B 级稠密模型，**在公开榜单上位居同尺寸第一**：平均分 43.56，覆盖推理、知识、数学、代码、指令跟随与 Agentic 工具调用。配套 [Agent Skills](./skills/) 覆盖主流推理与微调框架，支持一句话拉起。🔥🔥🔥
...
MiniCPM5 是面壁智能新一代端侧模型家族。首发的 **MiniCPM5-1B** 是一款紧凑稠密的 1B Transformer，定位于在 1B 参数量上把"每参数能力密度"做到最高，并配套 RL 后训练、单页 cookbook 与 面向 agent 的部署链路。
...
从 `MiniCPM5-1B-SFT` 出发，我们并行训练 **5 个专用 RL Teacher**：*Reasoning RL 1 / 2* 负责思维链准确性，*RLHF* 负责人类偏好，*IF RL* 负责指令跟随，*General RL* 负责综合能力，*Long Context RL* 负责长序列理解。随后通过 **Online Policy Distillation (OPD)** 统一蒸馏到同一个 student 模型。
...
MiniCPM5-1B 与同尺寸主流开源模型 **LFM2.5-1.2B-Thinking**、**Qwen3-0.6B/think**、**Qwen3.5-0.8B/think** 做了完整对比。23 项公开评测覆盖综合知识、领域知识、代码、指令跟随、数学推理、逻辑推理、主观写作和 Agentic 工具调用。**MiniCPM5-1B 是其中按总参数量最小的一档**，**平均分 43.56**，对比次优的 34.52 拉开显著差距，在 21 项已报告分数中拿下或并列 18 项第一。

![MiniCPM-5 1B 公开榜单成绩](./assets/minicpm5/public_leaderboard.png)

#### RL 增益

RL 训练是 MiniCPM5-1B 智能跃迁的最大单一来源：它把 SFT checkpoint 转化为推理 / 指令跟随任务中可用的助手。

![MiniCPM5-1B RL 增益](./assets/minicpm5/rl_gains.png)

聚焦数学、代码、指令跟随三类共 7 项评测，RL 训练带来 **↑16.0 分的平均增益**：数学（AIME 2025 ↑20.2 / AIME 2026 ↑25.6 / HMMT ↑11.6）和指令跟随（IFBench ↑17.1、Multi-IF ↑15.1、IFEval ↑10.7）提升最显著，LCB-v6 上也有 ↑11.6 的明显提升。

RL 同时让模型在推理任务上的**冗长程度大幅下降**：回复超出 max-tokens 预算被截断的比例平均**↓29 个百分点**（HMMT ↓40.5 pp、AIME 2025 ↓32.7 pp）。RL 不仅抬高了分数，还让推理链更短、更聚焦，对延迟敏感的端侧场景尤其重要。

![MiniCPM5-1B RL 超长率下降](./assets/minicpm5/rl_overlong.png)
...
在 Cursor / Claude Code 里扔一句话，agent 会自动选择对应的子 skill、配置环境、运行命令并回报结果：
...
MiniCPM5-1B 足够小、也足够强，可以作为桌宠的**本地 LLM 大脑**。我们提供了一个轻量桥接服务 **`minicpm-pet-bridge`**，把模型暴露为 OpenAI 兼容的本地端点，供 [Clawd on Desk](https://github.com/rullerzhou-afk/clawd-on-desk) 使用。Clawd on Desk 是一个像素桌宠，可以实时响应你的 AI 编码 agent。

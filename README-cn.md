...
🐱 **MiniCPM5 应用**：提供现成应用示例，包括以 MiniCPM5-1B 作为本地大脑的桌宠，以及社区共训的人设 LoRA 社区。详见 [MiniCPM5 应用](#minicpm5-应用)。
...
MiniCPM5-1B 以 XML 格式产出工具调用，两个引擎都需要自定义 parser 才能把它转成 OpenAI 兼容的 `tool_calls` 字段。

**SGLang** — 从 fork 安装（自带 `minicpm5` tool-call parser）：
...
插件代码见 [`tool_parsers/minicpm5xml_tool_parser.py`](./tool_parsers/minicpm5xml_tool_parser.py)。
